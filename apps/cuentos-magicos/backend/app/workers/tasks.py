...
def story_text_generation_task(self, story_id: str, tier_limits: dict, auth_headers: dict = None):
    ...
    try:
        asyncio.run(_run_text_generation(story_id, tier_limits, auth_headers or {}))
...
def generate_images_task(self, story_id: str, auth_headers: dict = None):
    ...
    try:
        asyncio.run(_run_images(story_id, auth_headers or {}))
...
def generate_audio_task(self, story_id: str, auth_headers: dict = None):
    ...
    try:
        asyncio.run(_run_audio(story_id, auth_headers or {}))
...
def generate_video_task(self, story_id: str, auth_headers: dict = None):
    ...
    try:
        asyncio.run(_run_video(story_id, auth_headers or {}))
...
async def _run_text_generation(story_id: str, tier_limits: dict, auth_headers: dict = None):
    ...
    async with get_async_session() as session:
        ...
        data = await generate_story_text(session, story, auth_headers)
        ...
        if "image" in parts and story.canonical_character_description:
            generate_images_task.delay(story_id, auth_headers)

        if "audio" in parts and story.text_generated:
            generate_audio_task.delay(story_id, auth_headers)

        if "video" in parts and tier_limits.get("video_enabled", False):
            generate_video_task.delay(story_id, auth_headers)
...
async def _run_images(story_id: str, auth_headers: dict = None):
    async with get_async_session() as session:
        await generate_images_for_story(session, story_id, auth_headers)
        await session.commit()
        await _check_and_mark_ready(session, story_id)


async def _run_audio(story_id: str, auth_headers: dict = None):
    async with get_async_session() as session:
        await generate_audio_for_story(session, story_id, auth_headers)
        await session.commit()
        await _check_and_mark_ready(session, story_id)


async def _run_video(story_id: str, auth_headers: dict = None):
    async with get_async_session() as session:
        await generate_video_for_story(session, story_id, auth_headers)
        await session.commit()
        await _check_and_mark_ready(session, story_id)
